...
writeFileSync(
  resolve(SOURCE_DIR, "katex-script.ts"),
  `export default ${JSON.stringify(katexScript)};`
);
...
const fontFiles = readdirSync(KATEX_FONT_DIR).filter((filename) =>
  fontTypeExpr.test(filename)
);
const fonts = [];
...
@font-face {
  ...
  src: url('data:application/font-woff2;base64,${fontData.toString(
    "base64"
  )}') format('${fontType}');
...
writeFileSync(
  resolve(SOURCE_DIR, "katex-style.ts"),
  `export default ${JSON.stringify(fontStyle + katexStyleClean)};`
);
